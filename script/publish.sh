...
helm package .
PACKAGE=$(ls | grep -i '^generic-helm-chart.*tgz$')
[[ ! -z "${PACKAGE}" ]] || usage "Helm package not found, exit 1"

echo "Uploading helm chart..."
RESP=$(curl --request POST --form "chart=@${PACKAGE}" --user $USER:$PASSWORD https://gitlab.int.hextech.io/api/v4/projects/645/packages/helm/api/alpha/charts)
echo $RESP
if [[ $RESP == *"20"* ]]; then
    echo "Upload succeed"
else
